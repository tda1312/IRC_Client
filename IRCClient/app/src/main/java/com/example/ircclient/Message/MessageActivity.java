...
//        View v = (View)inflater.inflate(R.layout.fragment_message, container, false);
        ...
        send = (Button) this.findViewById(R.id.button_chatbox_send); //The send button
        send.setEnabled(false);
        if(send == null) {
            Log.i("Send", "onCreateView:  send is null");
        }else{
            Log.i("TAG", "onCreateView: Send is not null");
        }
        final EditText chatBox = (EditText) this.findViewById(R.id.edittext_chatbox); //The chat box
        ...
        logs = (ListView) this.findViewById(R.id.message_container);
//        adapter2 = new LogsAdapter<>(getActivity(),items);
//        adapter2 = new LogsAdapter(getApplicationContext(), R.layout.fragment_message, items );
...
//        final Bundle bundle = intent.getStringExtra("name");
        send.setOnClickListener(new View.OnClickListener() { //When the button is clicked
            ...
            public void onClick(View v) {
                ...
                parseIn(chatBox.getText().toString() ,intent.getStringExtra("nick"));
                chatBox.getText().clear();
                Log.i("listenting", "onClick: Listening");
    ...
    public void startConnect() {
        Log.i("Starting connect", "startConnect: Sting connect");
//        Bundle bundle = this.getArguments();

        if (getIntent() != null) {
            String nick = getIntent().getStringExtra("nick");
            Log.i("nicck", "startConnect: "+ nick);
            String channel = getIntent().getStringExtra("channel");
            Log.i("nickk", "startConnect: "+ channel);
            String[] arr={"1", "2", "3", "4", "5", "6"};
            Random r=new Random();
            String port_no = arr[r.nextInt(arr.length)]+arr[r.nextInt(arr.length)]+arr[r.nextInt(arr.length)]+arr[r.nextInt(arr.length)];

            new ConnectTask().execute("irc.freenode.net", port_no, nick, channel);
